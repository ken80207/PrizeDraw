 ...
 * PROCESSED on success or FAILED on delivery failure.
 *
...
public class OutboxWorker(
    ...
    private suspend fun handleExchangeCompleted(event: OutboxEvent) {
        ...
        notificationService.sendPushBatch(
            listOf(
                PlayerId.fromString(initiatorId),
                PlayerId.fromString(recipientId),
            ),
            payload,
        )
    }
    ...
    private suspend fun handleExchangeRejected(event: OutboxEvent) {
        ...
        notificationService.sendPush(
            PlayerId.fromString(otherPlayerId),
            PushNotificationPayload(
        ...
        scope.launch {
            try {
                ...
                do {
                    ...
                    notificationService.sendPushBatch(playerIdValues, pushPayload)
