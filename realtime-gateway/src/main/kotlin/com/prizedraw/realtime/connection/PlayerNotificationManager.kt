 ...
 *
 * @param redisPubSub The pub/sub bus used for cross-pod fanout.
 */
public class PlayerNotificationManager(
     ...
     *
     * Removes the session from the player's session set. Cancels the Redis subscription
     * when the last session for the player is removed.
     *
    ...
    public fun unregister(
        ...
        log.debug(
            "Player {} notification session unregistered; remaining={}",
            playerId,
            sessionCount(playerId),
        )
    }
    ...
    public suspend fun broadcast(
        ...
        for (session in playerSessions) {
            ...
            try {
                ...
                log.warn(
                    "Failed to send notification to player {} session; removing: {}",
                    playerId,
                    e.message,
                )
                dead.add(session)
