 ...
 *
 * The server resolves the winning prize before any client animation starts to ensure
 * that the result is immutable during the animation and only revealed at [DrawSyncService.completeDraw].
 *
 ...
 *
 * CRITICAL: [Revealed] is the **only** event that carries the prize result.
 * All preceding events ([Started], [Progress], [Cancelled]) intentionally omit the result
 * to prevent spectators from seeing the outcome before the drawing player's animation completes.
 */
@Serializable
 ...
 *
 * The anti-spoiler mechanism works as follows:
 * 1. The caller supplies a [DrawResult] at [startDraw] time — the result is stored in the DB but
 ...
 *
 * This guarantees that spectators can never learn the result before the drawing player's
 * animation threshold is crossed, regardless of Redis message ordering.
 *
...
public class DrawSyncService(
     ...
     * @param animationMode Client animation type (e.g. `TEAR`, `SCRATCH`).
    ...
    public suspend fun startDraw(
        ...
        log.debug("Draw sync started: sessionId=${session.id} campaignId=$campaignId")
        return session
    }
     ...
     *
     * No-op if the session has already been revealed.
     *
     ...
     *
     * This is the only function that broadcasts [DrawSyncEvent.Revealed]. The caller is
     * responsible for executing the DB transaction (points debit, prize transfer) **after**
     * this call succeeds.
     *
