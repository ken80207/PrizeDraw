...
/**
 * Pre-computed draw result supplied by the caller before animation begins (draw-service copy).
 *
...
/**
 * Sealed hierarchy of draw-sync WebSocket events broadcast to spectators (draw-service copy).
 *
 * [Revealed] is the ONLY event that carries the prize result — all preceding events
 * intentionally omit the result to prevent spoilers.
 */
@Serializable
public sealed class DrawSyncEvent {
     ...
     *
     * Published only after [DrawSyncService.completeDraw] is called.
     */
    @Serializable
 ...
 *
 * Anti-spoiler flow:
 * 1. [startDraw] stores the pre-computed [DrawResult] in DB but does NOT include it in
 *    the [DrawSyncEvent.Started] broadcast.
 * 2. [relayProgress] fans out [DrawSyncEvent.Progress] events.
...
public class DrawSyncService(
     ...
     *
     * This is the only function that broadcasts [DrawSyncEvent.Revealed].
     *
