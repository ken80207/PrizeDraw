    ...
    FunSpec({
        ...
        test("POST draw kuji endpoint requires authentication") {
            testApplication {
                ...
                val response =
                    client.post(DrawEndpoints.DRAW_KUJI) {
        ...
        test("POST draw unlimited endpoint requires authentication") {
            testApplication {
                ...
                val response =
                    client.post(DrawEndpoints.DRAW_UNLIMITED) {
                        contentType(ContentType.Application.Json)
                        setBody("{}")
                    }
                response.status shouldBe HttpStatusCode.Unauthorized
        ...
        test("POST queue join endpoint requires authentication") {
            testApplication {
                ...
                val response =
                    client.post(DrawEndpoints.QUEUE_JOIN) {
                        contentType(ContentType.Application.Json)
                        setBody("{}")
                    }
                response.status shouldBe HttpStatusCode.Unauthorized
        ...
        test("DELETE queue leave endpoint requires authentication") {
            testApplication {
                ...
                val response =
                    client.delete(DrawEndpoints.QUEUE_LEAVE) {
                        contentType(ContentType.Application.Json)
                        setBody("{}")
                    }
                response.status shouldBe HttpStatusCode.Unauthorized
        ...
        test("POST sync progress endpoint requires authentication") {
            testApplication {
                ...
                val response =
                    client.post(DrawEndpoints.SYNC_PROGRESS) {
                        contentType(ContentType.Application.Json)
                        setBody("{}")
                    }
                response.status shouldBe HttpStatusCode.Unauthorized
